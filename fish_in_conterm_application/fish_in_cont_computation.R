...
fish_mat <- read.csv('fish_in_conterm/agap_fish_dataset.csv',
                       header=T, row.names = 1) %>%
  select(-(huc8 : sample_day))

freq <- colSums(fish_mat)
...
set.seed(1234)
fish_mat <- fish_mat[sample.int(L_tot, size = L_tot, replace = F),]

# Consider only the first 2000 as dataset, the others just for asymptotic discussion
fish_mat <- fish_mat[1:2000, ]

# number of sites
L = nrow(fish_mat)

fish_list <- create_features_list(fish_mat)
#plot_trajectory(fish_list)
...
save(params_poiss, file = "fish_results/fish_params_poiss.Rda")
save(params_negbin, file = "fish_results/fish_params_negbin.Rda")
save(params_ibp, file = "fish_results/fish_params_ibp.Rda")
save(params_sp, file = "fish_results/fish_params_sp.Rda")

############ 9) Save results: samples from limiting distributions (Poiss/NB) ##############
# Poisson
save(gg_ntilde_poiss, file = "fish_results/fish_ntilde_poiss.Rda")
# Negative Binomial
save(gg_ntilde_negbin, file = "fish_results/fish_ntilde_negbin.Rda")

######### 10) Save results: CI for extrapolation (Poiss/NB/Gamma/SP) ################
# Poisson
saveRDS(list_kmn_pred_test_poiss, "fish_results/fish_ci_poiss.rds")
# Negative Binomial
saveRDS(list_kmn_pred_test_negbin, "fish_results/fish_ci_negbin.rds")
# Gamma IBP
saveRDS(list_kmn_pred_test_ibp, "fish_results/fish_ci_ibp.rds")
# SB-SP
saveRDS(list_kmn_pred_test_sp, "fish_results/fish_ci_sp.rds")

######### 10.b) Save results: CI for rarefaction (Poiss/NB/Gamma/SP) ################
# Poisson
saveRDS(list_kn_rarefaction_poiss, "fish_results/fish_ci_insample_poiss.rds")
# Negative Binomial
saveRDS(list_kn_rarefaction_negbin, "fish_results/fish_ci_insample_negbin.rds")
# Gamma IBP
saveRDS(list_kn_rarefaction_ibp, "fish_results/fish_ci_insample_ibp.rds")
# SB-SP
saveRDS(list_kn_rarefaction_sp, "fish_results/fish_ci_insample_sp.rds")

######## 11) Save the data ##############################
saveRDS(data_mat, "fish_results/fish_data_mat.rds")
