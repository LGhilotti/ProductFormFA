...
colnames(gg_ntilde_poiss) <- paste("Ntilde", Ns, sep = ".")
gg_ntilde_negbin <- data.frame(matrix(nrow = number_saved_iterations_negbin, ncol = length(Ns)))
colnames(gg_ntilde_negbin) <- paste("Ntilde", Ns, sep = ".")
...
names(list_kmn_pred_test_poiss) <- paste("Ntilde", Ns, sep = ".")
list_kmn_pred_test_negbin <- vector(mode="list", length = length(Ns))
names(list_kmn_pred_test_negbin) <- paste("Ntilde", Ns, sep = ".")
list_kmn_pred_test_ibp <- vector(mode="list", length = length(Ns))
names(list_kmn_pred_test_ibp) <- paste("Ntilde", Ns, sep = ".")
...
for (j in 1:length(Ns)){
  ...
  gg_ntilde_poiss[[paste0("Ntilde.",N)]] <- ntilde_chain_poiss
  gg_ntilde_negbin[[paste0("Ntilde.",N)]] <- ntilde_chain_negbin
  ...
  list_kmn_pred_test_poiss[[paste0("Ntilde.",N)]] <- est_ci_pred_poiss
  ...
  list_kmn_pred_test_negbin[[paste0("Ntilde.",N)]] <- est_ci_pred_negbin
  ...
  list_kmn_pred_test_ibp[[paste0("Ntilde.",N)]] <- est_ci_pred_ibp
...
ggsave("chao_model_simulation/m1/m1_poiss_mix.pdf", width = 16, height = 12, units = "in")
...
ggs_traceplot(samples_ggs_negbin) + 
  facet_wrap(~Parameter, nrow = length(Ns), scales = "free")
ggsave("chao_model_simulation/m1_negbin_mix.pdf", width = 16, height = 12, units = "in")
...
ggs_traceplot(samples_ggs_ibp) + 
  facet_wrap(~Parameter, nrow = length(Ns), scales = "free")
ggsave("chao_model_simulation/m1_gamma_mix.pdf", width = 16, height = 12, units = "in")


############ 9) Plot limiting distributions (Poiss/NB) ##############
# Poisson
...
ggs_density(samples_ntilde_ggs_poiss) + 
  facet_wrap(~Parameter, nrow = length(Ns), scales = "free")
ggsave("chao_model_simulation/m1_poiss_limit.pdf", width = 12, height = 12, units = "in")
...
ggs_density(samples_ntilde_ggs_negbin) + 
  facet_wrap(~Parameter, nrow = length(Ns), scales = "free")
ggsave("chao_model_simulation/m1_negbin_limit.pdf", width = 12, height = 12, units = "in")

######### 10) Plot Extrapolation curve (Poiss/NB/Gamma) ################
# Poisson
ggarrange(plotlist = gg_kmn_pred_test_poiss)
ggsave("chao_model_simulation/m1_poiss_extr.pdf", width = 16, height = 8, units = "in")

# Negative Binomial
ggarrange(plotlist = gg_kmn_pred_test_negbin)
ggsave("chao_model_simulation/m1_negbin_extr.pdf", width = 16, height = 8, units = "in")

# Gamma IBP
ggarrange(plotlist = gg_kmn_pred_test_ibp)
ggsave("chao_model_simulation/m1_gamma_extr.pdf", width = 16, height = 8, units = "in")
...
###### 8) Plot limit distribution estimate #####
avg_ntilde_poiss_long <- gather(avg_ntilde_poiss, training, estimate, 
                                paste0("N.",Ns[2]):paste0("N.",Ns[length(Ns)]), factor_key=TRUE) %>%
  add_column(model = "Poiss", N = rep(Ns[2:length(Ns)], each = D))

avg_ntilde_negbin_long <- gather(avg_ntilde_negbin, training, estimate, 
                                paste0("N.",Ns[2]):paste0("N.",Ns[length(Ns)]), factor_key=TRUE) %>%
  add_column(model = "Neg-Bin", N = rep(Ns[2:length(Ns)], each = D))
