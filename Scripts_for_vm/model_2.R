...
c_fr <- 10
# Set the horizon for the rarefaction curve 
hors <- Ns*5
...
#   
#   ### 8) Rarefaction on the horizon set at the beginning ####
#   # Poisson
...
# 
# save(params_poiss, file = "m2_params_poiss.Rda")
# save(params_negbin, file = "m2_params_negbin.Rda")
# save(params_ibp, file = "m2_params_ibp.Rda")
# 
# ############ 9) Save results: samples from limiting distributions (Poiss/NB) ##############
# # Poisson
# save(gg_ntilde_poiss, file = "m2_ntilde_poiss.Rda")
# # Negative Binomial (fixed)
# save(gg_ntilde_negbin, file = "m2_ntilde_negbin.Rda")
# # Negative Binomial (prior)
# #save(gg_ntilde_negbin_prior, file = "m2_ntilde_negbin_prior.Rda")
# 
# ######### 10) Save results: CI for extrapolation (Poiss/NB/Gamma/SP) ################
# # Poisson
# saveRDS(list_kmn_pred_test_poiss, "m2_ci_poiss.rds")
# # Negative Binomial (fixed)
# saveRDS(list_kmn_pred_test_negbin, "m2_ci_negbin.rds")
# # Negative Binomial (prior)
# #saveRDS(list_kmn_pred_test_negbin_prior, "m2_ci_negbin_prior.rds")
# # Gamma IBP
# saveRDS(list_kmn_pred_test_ibp, "m2_ci_ibp.rds")
# # SB-SP
# #saveRDS(list_kmn_pred_test_sp, "m2_ci_sp.rds")
# 
# ######### 11) Save results: CI for rarefaction  ################
# # Poisson
# saveRDS(list_kn_rarefaction_poiss, "m2_ci_rare_poiss.rds")
# # Negative Binomial
# saveRDS(list_kn_rarefaction_negbin, "m2_ci_rare_negbin.rds")
# # Gamma IBP
# saveRDS(list_kn_rarefaction_ibp, "m2_ci_rare_ibp.rds")
# 
# 
# ######## 11) Save the data ##############################
# saveRDS(data_mat, "m2_data_mat.rds")
# 
...
# 
# list_chao_cov <- vector(mode="list", length = length(Ns))
# names(list_chao_cov) <- labels_comb_ibp
# 
# for (j in 1:length(Ns)){
#   
#   N <- Ns[j]
...
#   
#   chao_res_cov <- as_tibble(chao_res[["Cov"]]) %>%
#     rename(medians = Cov.hat, lbs = Norm.CI.Low, ubs = Norm.CI.High)
#   
#   list_chao_rare[[lab_comb]] <- as.data.frame(chao_res_rare)
#   list_chao_cov[[lab_comb]] <- as.data.frame(chao_res_cov)
#   
# }
# # Chao rarefaction
# saveRDS(list_chao_rare, "m2_chao_rare.rds")
# # Chao coverage
# saveRDS(list_chao_cov, "m2_chao_coverage.rds")
...
# estimated Kmn at last test observation (3 methods + gt + chao)
...
est_last_chao <- data.frame(matrix(nrow = D, ncol = length(Ns)))
colnames(est_last_chao) <- labels_comb_ibp
...
for (d in 1:D){
  ...
  for (j in 1:length(Ns)){
    ...
    Q_vec <- Q_vec[Q_vec>0]
    # Compute the curves with confidence intervals
    chao_res <- iNEXT.Sam(Spec = Q_vec, T = N, endpoint = L)
    
    chao_res_rare <- as_tibble(chao_res[["q=0"]]) %>%
      select(-Cov.hat) %>%
      rename(medians = D0.hat, lbs = Norm.CI.Low, ubs = Norm.CI.High)
    
    tmp <- as.data.frame(chao_res_rare)
    
    est_last_chao[d ,j ] <- tmp[nrow(tmp), "medians"]
...
save(avg_ntilde_poiss, file = "m2_avg_ntilde_poiss.Rda")
save(avg_ntilde_negbin, file = "m2_avg_ntilde_negbin.Rda")


###### 9) Save results: quantities for accuracy #####
save(obs_train, file = "m2_obs_train.Rda")
save(obs_new, file = "m2_obs_new.Rda")

save(est_new_poiss, file = "m2_est_new_poiss.Rda")
save(est_new_negbin, file = "m2_est_new_negbin.Rda")
save(est_new_ibp, file = "m2_est_new_ibp.Rda")
save(est_last_gt, file = "m2_est_last_gt.Rda")
save(est_last_chao, file = "m2_est_last_chao.Rda")
