...
gg_ntilde_poiss <- data.frame(matrix(nrow = number_saved_iterations_poiss, ncol = length(Ns)*length(Nbars)))
colnames(gg_ntilde_poiss) <- labels_comb
gg_ntilde_negbin <- data.frame(matrix(nrow = number_saved_iterations_negbin, ncol = length(Ns)*length(Nbars)))
...
list_kmn_pred_test_poiss <- vector(mode="list", length = length(Ns)*length(Nbars))
names(list_kmn_pred_test_poiss) <- labels_comb
list_kmn_pred_test_negbin <- vector(mode="list", length = length(Ns)*length(Nbars))
...
list_kmn_pred_test_ibp <- vector(mode="list", length = length(Ns)*length(Nbars))
...
params_poiss <- data.frame(matrix(nrow = number_saved_iterations_poiss, ncol = 2*length(Ns)*length(Nbars)))
colnames(params_poiss) <- paste(c("alpha", "theta"), rep(labels_comb, each = 2), sep = ":")
params_negbin <- data.frame(matrix(nrow = number_saved_iterations_negbin, ncol = 2*length(Ns)*length(Nbars)))
...
params_ibp <- data.frame(matrix(nrow = number_saved_iterations_ibp, ncol = 4*length(Ns)*length(Nbars)))
...
#save(params_negbin_prior, file = "unb_poly_0_8_params_negbin_prior.Rda")
...
#save(gg_ntilde_negbin_prior, file = "unb_poly_0_8_ntilde_negbin_prior.Rda")
...
# Negative Binomial (prior)
#saveRDS(list_kmn_pred_test_negbin_prior, "unb_poly_0_8_ci_negbin_prior.rds")
# Gamma IBP
...
# save(avg_ntilde_negbin_prior, file = "unb_poly_0_8_avg_ntilde_negbin_prior.Rda")
# 
# 
...
# save(est_new_negbin_prior, file = "unb_poly_0_8_est_new_negbin_prior.Rda")
