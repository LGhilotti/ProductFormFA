...
# BB mixtures use only EMPIRICAL hyperparameters
...
names(list_kn_rarefaction_sp) <- labels_comb_ibp

list_kmn_pred_test_poiss <- vector(mode="list", length = length(Ns))
names(list_kmn_pred_test_poiss) <- labels_comb_bb
list_kmn_pred_test_negbin <- vector(mode="list", length = length(Ns))
names(list_kmn_pred_test_negbin) <- labels_comb_bb
list_kmn_pred_test_ibp <- vector(mode="list", length = length(Ns))
names(list_kmn_pred_test_ibp) <- labels_comb_ibp
list_kmn_pred_test_sp <- vector(mode="list", length = length(Ns))
names(list_kmn_pred_test_sp) <- labels_comb_ibp

params_poiss <- data.frame(matrix(nrow = number_saved_iterations_poiss, ncol = 2*length(Ns)))
colnames(params_poiss) <- paste(c("alpha", "theta"), rep(labels_comb_bb, each = 2), sep = ":")
params_negbin <- data.frame(matrix(nrow = number_saved_iterations_negbin, ncol = 2*length(Ns)))
colnames(params_negbin) <- paste(c("alpha", "theta"), rep(labels_comb_bb, each = 2), sep = ":")
params_ibp <- data.frame(matrix(nrow = number_saved_iterations_ibp, ncol = 4*length(Ns)))
colnames(params_ibp) <- paste(c("a", "b", "alpha", "theta"), rep(labels_comb_ibp, each = 4), sep = ":")
params_sp <- data.frame(matrix(nrow = number_saved_iterations_ibp, ncol = 3*length(Ns)))
colnames(params_sp) <- paste(c("c", "beta", "alpha"), rep(labels_comb_ibp, each = 3), sep = ":")
...
for (j in 1:length(Ns)){
  ...
  lab_c <- paste0("c:",lab_comb_ibp)
  lab_beta <- paste0("beta:",lab_comb_ibp)
  ...
  params_poiss[[lab_alpha_bb]] <- alpha_chain_poiss
  params_poiss[[lab_theta_bb]] <- theta_chain_poiss
  ...
  params_negbin[[lab_alpha_bb]] <- alpha_chain_negbin
  params_negbin[[lab_theta_bb]] <- theta_chain_negbin
  ...
  params_ibp[[lab_alpha_ibp]] <- alpha_chain_ibp
  params_ibp[[lab_theta_ibp]] <- theta_chain_ibp
  ...
  if (max(alpha_chain_sp) - min(alpha_chain_sp) == 0 ){
    stop("SB-SP: error in mixing")
  }

  params_sp[[lab_c]] <- c_chain_sp
  params_sp[[lab_beta]] <- beta_chain_sp
  params_sp[[lab_alpha_ibp]] <- alpha_chain_sp
  ...
  gg_ntilde_poiss[[lab_comb_bb]] <- ntilde_chain_poiss
  gg_ntilde_negbin[[lab_comb_bb]] <- ntilde_chain_negbin
  ...
  list_kmn_pred_test_poiss[[lab_comb_bb]] <- est_ci_pred_poiss
  ...
  list_kmn_pred_test_negbin[[lab_comb_bb]] <- est_ci_pred_negbin
  ...
  list_kmn_pred_test_ibp[[lab_comb_ibp]] <- est_ci_pred_ibp

  # SB-SP
  kmn_chain_sp <- generate_Kmn_chain_gamma_ibp(c_chain_sp + 1,
                                               beta_chain_sp*(1-alpha_chain_sp)/alpha_chain_sp,
                                               alpha_chain_sp,
                                               1 - alpha_chain_sp,
                                               M = M, n = N, Kn)
  ...
  list_kmn_pred_test_sp[[lab_comb_ibp]] <- est_ci_pred_sp
...
save(params_poiss, file = "unb_poly_1_params_poiss.Rda")
save(params_negbin, file = "unb_poly_1_params_negbin.Rda")
save(params_ibp, file = "unb_poly_1_params_ibp.Rda")
save(params_sp, file = "unb_poly_1_params_sp.Rda")

############ 9) Save results: samples from limiting distributions (Poiss/NB) ##############
# Poisson
save(gg_ntilde_poiss, file = "unb_poly_1_ntilde_poiss.Rda")
# Negative Binomial (fixed)
save(gg_ntilde_negbin, file = "unb_poly_1_ntilde_negbin.Rda")

######### 10) Save results: CI for extrapolation (Poiss/NB/Gamma/SP) ################
# Poisson
saveRDS(list_kmn_pred_test_poiss, "unb_poly_1_ci_poiss.rds")
# Negative Binomial (fixed)
saveRDS(list_kmn_pred_test_negbin, "unb_poly_1_ci_negbin.rds")
# Gamma IBP
saveRDS(list_kmn_pred_test_ibp, "unb_poly_1_ci_ibp.rds")
# SB-SP
saveRDS(list_kmn_pred_test_sp, "unb_poly_1_ci_sp.rds")


######## 11) Save the data ##############################
saveRDS(data_mat, "unb_poly_1_data_mat.rds")
...
saveRDS(list_kmn_pred_test_gt, "unb_poly_1_gt_prediction.rds")
...
# mcmc parameters
S_poiss <- S_negbin <- 2*10^4
S_ibp <- 4*10^4
n_burnin_poiss <- n_burnin_negbin <- n_burnin_ibp<-  4*10^3
thin_poiss <- thin_negbin <- thin_ibp <- 2
seed <- 1234
number_saved_iterations_poiss <- (S_poiss - n_burnin_poiss)/thin_poiss
number_saved_iterations_negbin <- (S_negbin - n_burnin_negbin)/thin_negbin
...
for (d in 1:D){
  ...
  for (j in 1:length(Ns)){
    ...
    # Poisson
    kmn_chain_poiss <- generate_Kmn_chain_poiss_last(lambda_poiss, alpha_chain_poiss,
                                                     theta_chain_poiss, M = M, n = N)

    est_new_poiss[d , j] <- mean(kmn_chain_poiss)

    # Negative Binomial (fixed)
    kmn_chain_negbin <- generate_Kmn_chain_negbin_last(nstar_nb, p_nb,
                                                       alpha_chain_negbin, theta_chain_negbin,
                                                       M = M, n = N, Kn)
    ...
    # IBP + Gamma
    kmn_chain_ibp <- generate_Kmn_chain_gamma_ibp(a_chain_ibp, b_chain_ibp, alpha_chain_ibp,
                                                  theta_chain_ibp, M = M, n = N, Kn)[M,] # only the last subject
    ...
    kmn_chain_sp <- generate_Kmn_chain_gamma_ibp(c_chain_sp + 1,
                                                 ...
                                                 alpha_chain_sp,
                                                 1 - alpha_chain_sp,
                                                 M = M, n = N, Kn)[M,] # only the last subject
    ...
    est_last_gt[d, j] <- predict_good_toulmin(N, M, sfs, cts, alternative = 0)$preds [N+M+1]
...
save(avg_ntilde_poiss, file = "unb_poly_1_avg_ntilde_poiss.Rda")
save(avg_ntilde_negbin, file = "unb_poly_1_avg_ntilde_negbin.Rda")


###### 9) Save results: quantities for accuracy #####
save(obs_train, file = "unb_poly_1_obs_train.Rda")
save(obs_new, file = "unb_poly_1_obs_new.Rda")

save(est_new_poiss, file = "unb_poly_1_est_new_poiss.Rda")
save(est_new_negbin, file = "unb_poly_1_est_new_negbin.Rda")
save(est_new_ibp, file = "unb_poly_1_est_new_ibp.Rda")
save(est_new_sp, file = "unb_poly_1_est_new_sp.Rda")
save(est_last_gt, file = "unb_poly_1_est_last_gt.Rda")
