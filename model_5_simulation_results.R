...
ggsave("chao_model_simulation/m5/m5_poiss_extr.pdf", width = 16, height = 8, units = "in")

# Negative Binomial
ggarrange(plotlist = gg_kmn_pred_test_negbin)
ggsave("chao_model_simulation/m5/m5_negbin_extr.pdf", width = 16, height = 8, units = "in")

# Gamma IBP
ggarrange(plotlist = gg_kmn_pred_test_ibp)
ggsave("chao_model_simulation/m5/m5_ibp_extr.pdf", width = 16, height = 8, units = "in")

# SB-SP
ggarrange(plotlist = gg_kmn_pred_test_sp)
ggsave("chao_model_simulation/m5/m5_sp_extr.pdf", width = 16, height = 8, units = "in")
...
ggplot(joint_alt_long, aes( y=accuracy, fill=model)) + 
  ...
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5)) +
  ggtitle(paste0("Model 5: accuracy (alt) on D = ", D)) +
  scale_y_continuous(breaks = pretty_breaks()) 
