...
###### check mcmc mixing Gamma ibp -> not used 
samples_ibp <- mcmc.list(mcmc(params_ibp))
samples_ggs_ibp <- ggs(samples_ibp, keep_original_order = TRUE)
ggs_traceplot(samples_ggs_ibp) 
...
###### 6) Rarefaction curves
gg_kn_rarefaction_all <- plot_Kn_median_and_rarefaction_all_sp_noibp(
  train_list = data_list,
  ci_poiss = kn_rarefaction_poiss, 
  ci_negbin = kn_rarefaction_negbin,
  ci_sp = kn_rarefaction_sp,
  n_avg = 200) +
  ggtitle(paste0("n = ", L)) + theme(plot.title = element_text(size=12)) 

ggsave(filename = "Plots_paper/plot_bci_op_rare.png", width = 8, height = 5, dpi = 300, units = "in", device='png')
...
###### 7.e) Prediction on different horizons, all together (results)  ########
list_kmn_pred_poiss_small <- readRDS(file = "bci_results/bci_op_small_ci_poiss.rds")
list_kmn_pred_negbin_small <- readRDS(file = "bci_results/bci_op_small_ci_negbin.rds")
list_kmn_pred_ibp_small <- readRDS(file = "bci_results/bci_op_small_ci_ibp.rds")
list_kmn_pred_sp_small <- readRDS(file = "bci_results/bci_op_small_ci_sp.rds")

list_kmn_pred_poiss_large <- readRDS(file = "bci_results/bci_op_1000_ci_poiss.rds")
list_kmn_pred_negbin_large <- readRDS(file = "bci_results/bci_op_1000_ci_negbin.rds")
list_kmn_pred_ibp_large <- readRDS(file = "bci_results/bci_op_1000_ci_ibp.rds")
list_kmn_pred_sp_large <- readRDS(file = "bci_results/bci_op_1000_ci_sp.rds")

list_kmn_pred_poiss <- c(list_kmn_pred_poiss_small,list_kmn_pred_poiss_large)
list_kmn_pred_negbin <- c(list_kmn_pred_negbin_small,list_kmn_pred_negbin_large)
list_kmn_pred_ibp <- c(list_kmn_pred_ibp_small, list_kmn_pred_ibp_large)
list_kmn_pred_sp <- c(list_kmn_pred_sp_small,list_kmn_pred_sp_large)

Ts <- c(length(list_kmn_pred_poiss$T.200$medians),
        length(list_kmn_pred_poiss$T.600$medians),
        length(list_kmn_pred_poiss$T.1000$medians))
...
for (j in 1:length(Ts)){
  ...
  gg_kmn_pred_all[[j]] <- plot_Kmn_median_pred_all_sp_noibp(
    data_list = data_list,
    ci_poiss = list_kmn_pred_poiss[[paste0("T.",hor)]], 
    ci_negbin = list_kmn_pred_negbin[[paste0("T.",hor)]],
    ci_sp = list_kmn_pred_sp[[paste0("T.",hor)]],
    n_avg = 100) +
    ggtitle(paste0("# new observations = ", hor)) + theme(plot.title = element_text(size=12))
...
ggsave(filename = "Plots_paper/plot_bci_op_all_pred.png", width = 10, height = 5, dpi = 300, units = "in", device='png')
...
ggsave(filename = "Plots_paper/plot_bci_op_all_richness.png", width = 8, height = 5, dpi = 300, units = "in", device='png')
