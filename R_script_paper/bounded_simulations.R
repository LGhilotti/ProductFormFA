...
if (!file.exists(paste0("R_script_paper/eb_EFPF_",mechanism,"_fit_estimate.RData"))) {
  ...
  vars_fct_NegBinBB <- c(10, 100) # c(10, 1000) 
  ...
  eb_EFPF_fit_estimate_bounded_scenario(mechanism = mechanism, 
                                        ...
                                        eb_init_BB, eb_known_BB,
                                        eb_init_IBP, eb_known_IBP,
                                        seed = 123456) # seed = 123 for "custom", seed = 123456 for "beta_pis"
...
ggsave(filename = paste0("R_script_paper/Paper_plots/rarefaction_",mechanism, "_eb_EFPF_means.pdf"), width = 4, height = 4, dpi = 300, units = "in", device='pdf')
...
plot_rare_ci
ggsave(filename = paste0("R_script_paper/Paper_plots/rarefaction_", mechanism,"_eb_EFPF_credible_int.pdf"), width = 5.2, height = 5.2, dpi = 300, units = "in", device='pdf')
...
plot_knr_ci
ggsave(filename = paste0("R_script_paper/Paper_plots/knr_", mechanism, "_eb_EFPF_credible_int.pdf"), width = 5.2, height = 5.2, dpi = 300, units = "in", device='pdf')
...
if (!file.exists(paste0("R_script_paper/eb_Freq_",mechanism,"_fit_estimate.RData"))) {
  ...
  # Save the entire workspace related to the mechanism just performed
  save(M, list_rare_extr_Chao,
       list_extr_GT, 
       file =  paste0("R_script_paper/eb_Freq_",mechanism,"_fit_estimate.RData"))
